...
export function prepareCanonicalTransferArchiveWrite(input={},options={}){
  ...
  const business={event_type:'bed_transfer',type:'TF',from_bed:clean(v.from_bed),to_bed:clean(v.to_bed),transfer_at:clean(v.transfer_at),transfer_reason:clean(v.transfer_reason),corpid:clean(v.corpid),source_context_anchor_refs:sorted(v.source_context_anchor_refs),carried_arrears_refs:sorted(v.carried_arrears_refs),rent_coverage_ref:clean(v.rent_coverage_ref),deposit_context_ref:clean(v.deposit_context_ref),expiry_context_ref:clean(v.expiry_context_ref),access_snapshot_fingerprint:clean(v.snapshot_fingerprint||v.access_snapshot_fingerprint),ttlock_sequence:clean(v.ttlock_sequence),source_snapshot_fingerprint:clean(v.source_snapshot_fingerprint),target_snapshot_fingerprint:clean(v.target_snapshot_fingerprint),ttlock_observation_at:clean(v.ttlock_observation_at),physical_state_before_submission:v.physical_state_before_submission||{},continuity_checks:v.continuity_checks||{},reconciliation_required:v.reconciliation_required===true,fee_mode:clean(v.fee_mode),fee_amount_aed:Number(v.fee_amount_aed||0),fee_due_date:clean(v.fee_due_date),fee_waiver_reason:clean(v.fee_waiver_reason),payment_method:clean(v.payment_method)};
