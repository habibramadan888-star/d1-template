...
export function prepareOwnerBedTransferVoid(input={},options={}){
  ...
  const allowed=new Set(['transfer_anchor_id','reason','idempotency_key']);
  ...
  const acceptedAt=clean(input.accepted_at),owner=clean(input.owner_reference),fingerprint=hash({corpid:clean(input.corpid),target,reason});
