...
| QA date/time                        | Autofill generated `2026-05-25T10:56:36+04:00`; actual QA run time MANUAL_REQUIRED                                                                                                           |
| Reviewer                            | MANUAL_REQUIRED                                                                                                                                                                              |
| Branch                              | `nightshift/v4-commercialization-safe-run`                                                                                                                                                   |
| Commit                              | `1f1f36b`                                                                                                                                                                                    |
| Staging Worker URL                  | MANUAL_REQUIRED - not found in committed non-secret config                                                                                                                                   |
...
| Staging D1 name                     | MANUAL_REQUIRED - configs bind `DB` to `homelink`; read-only `wrangler d1 list --json` did not show a confirmed `homelink-staging` database                                                  |
| Staging KV namespace                | MANUAL_REQUIRED - configs bind `RATE_LIMIT` to one namespace id, but no confirmed staging KV namespace was found                                                                             |
| Backup completed before write tests | MANUAL_REQUIRED - no backup executed by this autofill task                                                                                                                                   |
