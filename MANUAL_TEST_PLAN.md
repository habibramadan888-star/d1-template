...
| M-003 | Employee auth         | Login employee locally                                  | local `.dev.vars` employee credentials | login succeeds, cookie set                         | PASS in authenticated smoke      | Yes  | P0   |
| M-004 | Owner auth            | Login owner locally                                     | local `.dev.vars` owner credentials    | login succeeds, manager role returned              | PASS in authenticated smoke      | Yes  | P0   |
| M-005 | Staff permission      | Employee calls `/api/history`                           | employee session                       | 403 forbidden                                      | PASS in authenticated smoke      | Yes  | P0   |
| M-006 | Owner permission      | Owner calls `/api/history`                              | owner session                          | 200 list                                           | PASS in authenticated smoke      | Yes  | P1   |
| M-007 | Rent short pay        | Submit short-paid rent entry                            | local employee session                 | transaction + arrear task                          | FAIL: missing transactions table | No   | P0   |
...
| M-021 | Auth/API smoke        | Run `npm run smoke:core` against local/staging Worker   | ignored local/staging credentials      | owner APIs pass; employee APIs pass/deny correctly | PASS locally                     | Yes  | P0   |
| M-022 | Clean bootstrap probe | Run `npm run probe:clean-bootstrap`                     | disposable local D1 state              | employee entry succeeds on clean local D1          | FAIL: missing `transactions`     | No   | P0   |
