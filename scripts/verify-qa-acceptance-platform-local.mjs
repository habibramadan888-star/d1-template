...
export async function verifyQaAcceptancePlatformLocal() {
  ...
  try {
    ...
    const created = await req(base, "/api/qa/acceptance/runs", { method: "POST", headers: { Cookie: ownerCookie }, body: JSON.stringify({ mode: "quick" }) }); assert.equal(created.status, 200, JSON.stringify(created.body));
    ...
    const automation = await req(base, `/api/qa/acceptance/runs/${runId}/automation`, { method: "POST", headers: { Cookie: ownerCookie }, body: JSON.stringify({ aggregate_http_status: 200, validation_results: aggregate.body.validation_results.map(row => ({ entry_identity: row.entry_identity, ok: row.ok })), formal_write_count: 0, ttlock_external_calls: 0 }) }); assert.equal(automation.status, 200); assert.equal(automation.body.status, "AUTOMATION_PASS");
