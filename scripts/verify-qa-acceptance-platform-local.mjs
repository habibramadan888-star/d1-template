...
export async function verifyQaAcceptancePlatformLocal() {
  ...
  try {
    ...
    const aggregate = await req(base, "/api/employee/entry/validate", { method: "POST", headers: { Cookie: staffCookie }, body: JSON.stringify({ aggregate_preflight: true, validation_requests: validationRequests, dry_run: true, validate_only: true, no_write: true, diagnostic: { frontend_asset_version: "qa-validation-provenance-v1" }, qa_validation_context: qaValidationContext }) }); assert.equal(aggregate.status, 200, JSON.stringify(aggregate.body)); assert.equal(aggregate.body.validation_result_count, 16); assert.equal(aggregate.body.formal_write_count, 0);
    const automation = await req(base, `/api/qa/acceptance/runs/${runId}/automation`, { method: "POST", headers: { Cookie: staffCookie }, body: JSON.stringify({ ...qaValidationContext, validation_attempt_id: aggregate.body.validation_attempt_id, trace_id: aggregate.body.trace_id, aggregate_http_status: 200, validation_results: aggregate.body.validation_results.map(row => ({ entry_identity: row.entry_identity, event_type: row.event_type, ok: row.ok, error_code: row.error_code || "", missing_fields: row.missing_fields || [], diagnostic_envelope: row.diagnostic_envelope })), formal_write_count: 0, ttlock_external_calls: 0 }) }); assert.equal(automation.status, 200, JSON.stringify(automation.body)); assert.equal(automation.body.status, "AUTOMATION_PASS");
