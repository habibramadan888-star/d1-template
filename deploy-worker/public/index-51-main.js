...
function ccRender(forceRebuild=false){
  ...
  if(billing){
    const p=projection.period||{},status=projection.source_status||{},proof=projection.source_proof||{};
    const observed=proof.occupancy_observed_at?` · 门禁观测 ${esc(String(proof.occupancy_observed_at).replace('T',' ').slice(0,16))}`:'';
    const staleNote=status.occupancy==='stale'?'<div style="margin-top:8px;padding:8px 10px;border-radius:8px;background:rgba(224,108,0,.08);color:var(--orange);font-size:10px">当前展示最近一次可靠门禁快照，仅供查看；网络权限保持人工复核，不自动变更。</div>':'';
    billing.innerHTML=`<div class="card billing-widget"><div class="card-body" style="padding:16px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start"><div><div style="font-weight:800;font-size:15px">个人履约信用</div><div style="font-size:10px;color:var(--text3);margin-top:3px">观察账期 ${esc(p.start||'')} → ${esc(p.end||'')} · ${esc(projection.projection_version||'')}</div></div><button class="btn btn-ghost" style="font-size:11px;padding:5px 9px" onclick="ccRecomputeClientCredit()">刷新</button></div><div style="margin-top:10px;font-size:11px;color:var(--text2);line-height:1.7">仅依据个人付款及时性、个人未结清与复核状态形成简洁信用结论；账期收入不参与信用。</div><div style="margin-top:8px;font-size:10px;color:var(--text3)">历史 ${esc(status.history||'unknown')} · 门禁 ${esc(status.occupancy||'unknown')} · 租金 ${esc(status.rent||'unknown')} · 欠款 ${esc(status.arrears||'unknown')} · 浏览器缓存未参与${observed}</div>${staleNote}</div></div>`;
  }
