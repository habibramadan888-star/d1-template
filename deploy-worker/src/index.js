...
async function handleEmployeeRawIngestionEntry(request,env,user,options={}){
  const request_context=options.request_context||{
    started_at_ms:Date.now(),
    route_category:"employee_raw_ingestion",
    d1_read_count:0,
    d1_write_count:0
  };
  ...
  const rawValidation=employeeRawIngestionValidationResult(rawBody,{
    ok:true,
    stage:"raw_ingestion_preflight",
  ...
  },Number(rawBody?.event_index||0)||0);
  return persistEmployeeRawIngestion(env,user,rawBody,rawValidation,request_context);
}
__name(handleEmployeeRawIngestionEntry,"handleEmployeeRawIngestionEntry");
async function handleEmployeeEntry(request,env,user,options={}){
  ...
  return handleEmployeeRawIngestionEntry(request,env,user,{...options,body});
}
