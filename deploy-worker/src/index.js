...
async function persistEmployeeBedTransferCanonicalArchive(env,user,body,validationResult,requestContext=null){
  ...
  const sessionColumns=await empTableColumns(env,"sessions",requestContext).catch(()=>new Set());
  if(!(sessionColumns instanceof Set)||!sessionColumns.has("entries_json"))return json({success:false,ok:false,error_code:"CANONICAL_ARCHIVE_SCHEMA_UNAVAILABLE",no_write:true,write_attempted:false,production_cutover:"PRODUCTION_NO_GO"},503);
  const summaryJsonAvailable=sessionColumns.has("summary_json");
  ...
  if(existing){
    ...
    const updateValues=[mergedJson,mergedEntries.length,mergedSummaryFields.cash_handover,mergedSummaryFields.bank_transfer_total,mergedSummaryFields.bank_transfer_count,mergedSummaryFields.gross_received];
    if(summaryJsonAvailable)updateValues.push(mergedSummaryFields.summary_json);
    updateValues.push(prepared.entry.canonical_accepted_at,sessionId,user.corpid);
    const update=await env.DB.prepare(`UPDATE sessions SET entries_json=?, entries_count=?, cash_handover=?, bank_transfer_total=?, bank_transfer_count=?, gross_received=?${summaryJsonAvailable?", summary_json=?":""}, handover_status='COMPLETED', exported_at=? WHERE id=? AND corpid=? AND COALESCE(voided_at,'')='' AND UPPER(COALESCE(handover_status,''))='EXPORTING'`).bind(...updateValues).run();
  ...
  try{
    const insertColumns=["id","corpid","anchor_id","date","entries_count","created_by","created_at","operator_id","operator_name","cash_handover","bank_transfer_total","bank_transfer_count","gross_received","handover_status","exported_at","export_text","source","entries_json"];
    const insertValues=[sessionId,user.corpid,prepared.entry.transfer_anchor_id,cleanDate(session.date||prepared.entry.canonical_accepted_at),1,user.userid,prepared.entry.canonical_accepted_at,user.userid,cleanText(user.employee_name||user.userid,120),preparedSummaryFields.cash_handover,preparedSummaryFields.bank_transfer_total,preparedSummaryFields.bank_transfer_count,preparedSummaryFields.gross_received,'COMPLETED',prepared.entry.canonical_accepted_at,'','employee_entry',prepared.entries_json];
    if(summaryJsonAvailable){insertColumns.push("summary_json");insertValues.push(preparedSummaryFields.summary_json);}
    await env.DB.prepare(`INSERT INTO sessions (${insertColumns.join(",")}) VALUES (${insertColumns.map(()=>"?").join(",")})`).bind(...insertValues).run();
