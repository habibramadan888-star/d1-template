...
| P1-006 | Embedded worker drift           | `src/index.js` and `src/index.embedded.js` can drift           | P1     | Partial     | No                 | Drift audit and dry-run gate added         | `DEPLOY_ENTRYPOINT_REVIEW.md`, `WORKER_ENTRYPOINT_DRIFT_AUDIT.md`, `EMBEDDED_WORKER_FRESHNESS_RESULT.md`, `EMBEDDED_WORKER_GENERATION_DRY_RUN_RESULT.md`, `scripts/audit-worker-entrypoint-drift.mjs`, `scripts/verify-embedded-worker-freshness.mjs`, `scripts/generate-embedded-worker-dry-run.mjs`, `npm run audit:worker-drift`, `npm run verify:embedded-worker`, `npm run build:embedded:dry-run` | Embedded artifact is stale for P0-002C; controlled write and deploy entrypoint approval are still required before staging/prod deploy | Human must approve deploy entrypoint and controlled embedded write before embedded staging/prod deploy |
