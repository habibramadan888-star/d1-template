...
- Selected TTLock rooms/beds were exactly `112`, `113`, and `125`.
- Owner read model showed TTLock expired unpaid rows with task IDs `ttlock-expired-139777220`, `ttlock-expired-139777824`, and `ttlock-expired-139778918`.
- Readiness failed because the real directive endpoint only updates persisted `arrear_tasks` rows, while the selected TTLock rows are generated by the read-only aggregation path and are not confirmed as persisted write rows.
- Write gate was not opened.
- Owner directive create was not called.
- Employee follow-up was not called.
- Abdul inbox baseline remained 1 persisted directive; no +3 production delivery occurred.
- No production D1 write, migration, D1 execute/export/import, batch dispatch, TTLock full rollout, financial formula change, dashboard calculation change, or production cutover occurred.
- Production cutover remains `PRODUCTION_NO_GO`.
...
- Production cutover remains `PRODUCTION_NO_GO`.
## Owner Arrears Batch Display / Export Update

- Added current-filter select-all for owner arrears tasks.
- Removed the directive date input from the owner arrears action bar.
- Converted send-employee action to dry-run execution-list generation; no D1 write or real directive write is executed.
- Simplified filters to: all, TTLock expired, existing arrears.
