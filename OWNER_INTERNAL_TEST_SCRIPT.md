...
Scope: owner manual QA planning. Local asset review found no separate
`owner.html`, `boss.html`, or `dashboard.html`; the owner/boss interface is the
main SPA. Use `https://homelink-finance.habibramadan888.workers.dev/` or the
explicit asset `https://homelink-finance.habibramadan888.workers.dev/index.html`
for owner login. Local wrangler config binds this Worker to `DB = homelink`, so
write-style tests can affect production data and must not be run unless
production D1 writes are separately approved. Do not include passwords, tokens,
cookies, or real production customer data in evidence.

| Test ID | Scenario                            | Steps                                                                                                                           | Expected Result                                                                       | Evidence Needed                                                    | Pass/Fail | Notes |
| ------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | --------- | ----- |
| OWN-001 | Login / logout                      | Open `https://homelink-finance.habibramadan888.workers.dev/`, login as owner/manager, confirm dashboard/history access, logout. | Owner can login/logout and employee-only write controls are not exposed unexpectedly. | Screenshot after login and logout with secrets masked.             |           |       |
