...
interface is the main SPA. Start from
`https://homelink-finance.habibramadan888.workers.dev/unified-login.html`; the
server-confirmed owner/manager/admin role must route to `index.html`. Local
...
| ------- | ----------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | --------- | ----- |
| OWN-001 | Login / logout                      | Open unified login, sign in as owner/manager, confirm automatic route to `index.html`, then logout. | Owner can login/logout and employee-only write controls are not exposed unexpectedly. | Screenshot after login and logout with secrets masked.             |           |       |
