...
test("owner lock/logout clears auth state and routes only to unified-login", async () => {
...
test("employee auth routing also uses unified-login for sign-out/session failure", async () => {
  const html = await readFile("deploy-worker/public/employee-v3.html", "utf8");

  assert.match(html, /const UNIFIED_LOGIN_DESTINATION='\.\/unified-login\.html'/);
