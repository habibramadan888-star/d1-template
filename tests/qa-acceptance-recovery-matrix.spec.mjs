...
import { QA_FULL_FINANCE_EXPECTED, QA_RECOVERY_SCENARIOS, qaAcceptanceMatrix } from "./fixtures/employee-qa-acceptance-matrices.mjs";
...
test("Recovery uses the same 44-entry Full business oracle and freezes the four intended missing ordinals", () => {
  const matrix = qaAcceptanceMatrix("recovery");
  assert.equal(matrix.scenarios.length, 44);
  assert.deepEqual(matrix.expected_finance, QA_FULL_FINANCE_EXPECTED);
  assert.equal(matrix.recovery_plan.initial_persisted_count, 40);
  assert.deepEqual(matrix.recovery_plan.initial_missing_ordinals, [25, 27, 43, 44]);
  assert.equal(matrix.recovery_plan.fault, "response_lost_after_initial_persisted");
});
