...
test("unauthenticated employee-v3 redirects to unified login instead of showing old PIN UI", async () => {
...
test("unauthenticated owner index redirects to unified login instead of old owner login", async () => {
  const js = await readFile("deploy-worker/public/index-51-main.js", "utf8");

  assert.match(js, /const UNIFIED_LOGIN_DESTINATION='\.\/unified-login\.html'/);
...
test("authenticated roles route to the correct business destination", async () => {
  ...
  assert.match(
    employee,
    /if\(isOwnerAuthRole\(me\.role\)\)\{\s*location\.replace\('\.\/index\.html'\)/
  );
  assert.match(
    owner,
    /if\(isEmployeeAppRole\(me\.role\)\)\{\s*location\.replace\('\.\/employee-v3\.html'\)/
  );
