...
- P0-001: Partial. P0-001A now documents the money field inventory, finance flow map, AED fils policy, helper design, and migration plan. `npm run test:money` passes and `npm run audit:money` reports 187 REAL/FLOAT risks, 466 JS Number/parseFloat risks, 435 frontend money calculation risks, and 143 backend money calculation risks after adding P0-001B shadow artifacts. P0-001B added read-only local D1 shadow reconciliation; `npm run reconcile:money` currently scans 22 money columns and 0 non-null local legacy values. Legacy runtime still uses `REAL`/`Number`, so P0-001 is not Verified.
