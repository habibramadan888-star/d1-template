...
| P1-006 | Worker         | `src/index.embedded.js` is generated/stale relative to source.                       | Embedded deploy path can drift from source behavior. | Regenerate only in a controlled deploy-prep step, not during audit. | P1-006 added route/guard drift audit, freshness hash gate, dry-run embedded generation to `.tmp`, and deploy-artifact GO/NO-GO docs. Current embedded artifact is stale for P0-002C and cannot be used for staging/prod until controlled write is approved. |
