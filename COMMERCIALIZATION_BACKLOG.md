...
| P0-005 | Database     | Clean D1 bootstrap is incomplete or unproven.                                              | New customer environment may fail at runtime.                    | Build ordered migrations for all business tables.                                             | New local D1 starts from zero and passes smoke/flow tests.       |
...
| P0-007 | Auth         | Authenticated local employee/owner flows are blocked without local non-production secrets. | Staff/owner permission paths are not fully verified.             | Add documented local `.dev.vars` setup and hash helper, without committing secrets.           | Login tests pass locally.                                        |
...
1. Configure local non-production secrets and authenticated smoke tests.
