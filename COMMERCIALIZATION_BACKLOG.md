...
| P1-007 | API            | API inventory now has a local drift gate, but hosted CI enforcement is not configured. | Route docs can still drift outside local checks.     | Run `npm run check` in CI before any merge/deploy.                  | CI blocks route metadata drift.                      |
