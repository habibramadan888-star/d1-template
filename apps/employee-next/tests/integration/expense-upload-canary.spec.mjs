...
test("Expense upload stays hidden without exact authenticated STAFF and one Expense", async () => {
  for (const options of [
    { restoreError: true },
    { session: staff("EMPLOYEE") },
    { session: staff("OWNER") },
    { session: staff("") },
  ...
  try {
    ...
    assert.equal(await nonExpense.sidecar.uploadExpense(), false);
  ...
  try {
    ...
    assert.equal(await multiple.sidecar.uploadExpense(), false);
...
test("one confirmed Expense produces exactly one POST and only explicit success becomes SYNCED", async () => {
  ...
  try {
    ...
    assert.equal(fixture.adapter.calls[0].body.entry.event_type, "expense");
    assert.deepEqual(fixture.sidecar.getExpenseUploadState(), {
      status: "SYNCED",
      entryId: "cloud-expense-entry",
      sessionId: "cloud-expense-session",
    });
